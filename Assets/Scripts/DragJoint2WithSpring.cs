...
public class DragJoint2WithSpring : MonoBehaviour
{
    // Candidates for public
    private float _fps = 15;
    private float SnapScale = 4;

    // New variables with Howard's equations
    private float _speed;
    private float _kineticCoefficient = 1.2f;
    private float _dampingCoefficient = 0.2f;
    ...
    private void Update()
    {
        ...
        if (_reset && _timer <= 0 && Mathf.Abs(_currentScaleValue - _originalScaleValue) > 0.001f)
        {
            ...
            _speed = _speed - _kineticCoefficient * (_currentScaleValue - _originalScaleValue) - _dampingCoefficient* _speed;
